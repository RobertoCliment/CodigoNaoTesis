%%En este test se observa la diferencia del ruido en el proceso con
%%el valor real
...
while(Xtk(1)>0.05)
    ...
    %Gráfica de la diferencia entre la posición con y sin ruido
    figure(1)
    title('Diferencia entre la posición con y sin ruido')
    plot(i*DeltaT, XtkRuido(1)-Xtk(1), 'ob')
    axis([0 10 -1 1])
    hold on
    
    %Gráfica de la diferencia entre la velocidad con y sin ruido
    figure(2)
    title('Diferencia entre la velocidad con y sin ruido')
    plot(i*DeltaT, XtkRuido(2)-Xtk(2), '*b')
    axis([0 10 -1 1])
    hold on
